...
function git(cwd: string, ...args: string[]): string {
  return execFileSync('git', ['-C', cwd, ...args], { stdio: 'pipe', encoding: 'utf-8' }).trim();
}
...
export async function createWorktree(agentId: string, repo?: string, agentName?: string): Promise<WorktreeResult> {
  ...
  try {
    ...
    execFileSync('git', ['-C', repo, 'worktree', 'add', worktreePath, '-b', branchName], { stdio: 'pipe' });
    return { path: worktreePath, created: true };
  } catch (err) {
    fs.mkdirSync(worktreePath, { recursive: true });
...
export async function removeWorktree(agentId: string, repo?: string): Promise<void> {
  ...
  if (repo && fs.existsSync(worktreePath)) {
    try {
      execFileSync('git', ['-C', repo, 'worktree', 'remove', worktreePath, '--force'], { stdio: 'pipe' });
    } catch {
      try {
  ...
  if (repo) {
    try {
      execFileSync('git', ['-C', repo, 'branch', '-D', `boardroom/${agentId}`], { stdio: 'pipe' });
    } catch {}
  }
}
...
export function getWorktreeGitInfo(worktreePath: string, repo?: string): GitInfo {
  ...
  if (repo) {
    ...
    for (const candidate of ['main', 'master', 'develop']) {
      if (gitSafe(worktreePath, 'rev-parse', '--verify', candidate)) {
  ...
  const logRangeArgs = baseBranch ? [`${baseBranch}..${branch}`] : ['-10'];
  const logOut = gitSafe(worktreePath, 'log', ...logRangeArgs, '--format=%H|%s|%an|%ci', '--no-merges');
  if (logOut) {
