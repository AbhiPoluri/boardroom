...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ git: info, diff });
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!info.isGit || !info.branch) {
    return NextResponse.json({ error: 'Not a git worktree' }, { status: 400 });
  }
  ...
  switch (action) {
    ...
    case 'cherry-pick': {
      if (!commits?.length) return NextResponse.json({ error: 'No commits specified' }, { status: 400 });
      const result = cherryPickCommits(agent.repo, commits, baseBranch);
      return NextResponse.json(result);
    }
    case 'patch': {
      const patch = createPatch(agent.worktree_path!, info.baseBranch);
      if (!patch) return NextResponse.json({ error: 'No patch generated' }, { status: 400 });
